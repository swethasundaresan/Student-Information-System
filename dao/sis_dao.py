from exception.custom_exception import StudentNotFoundException, TeacherNotFoundException, CourseNotFoundException, \
    PaymentValidationException, InvalidStudentDataException, InvalidCourseDataException, InvalidTeacherDataException
...
class SIS(DBConnection):
    ...
    def UpdateStudentInfo(self, student_id, new_first_name, new_last_name, new_date_of_birth, new_email,
                          new_phone_number):
        try:
            self.open()
            ...
            self.c.execute(
                f"""INSERT INTO Students (student_id, first_name, last_name, date_of_birth, email,phone_number) VALUES ({student_id},'{new_first_name}', '{new_last_name}', '{new_date_of_birth}','{new_email}','{new_phone_number}')""")
            self.mydb.commit()
    ...
    def AssignTeacher(self, course_id, teacher_id):
        try:
            self.open()
            query_course_existence = f"SELECT * FROM courses WHERE course_id = {course_id}"
            self.c.execute(query_course_existence)
            course_exists = self.c.fetchone()

            if not course_exists:
                raise CourseNotFoundException(f"Course with ID {course_id} does not exist.")

            query_teacher_existence = f"SELECT * FROM teacher WHERE teacher_id = {teacher_id}"
            self.c.execute(query_teacher_existence)
            teacher_exists = self.c.fetchone()

            if not teacher_exists:
                raise TeacherNotFoundException(f"Teacher with ID {teacher_id} does not exist.")

            # Proceed with assigning the teacher to the course
            query_assign_teacher = f"""
                            UPDATE courses
                            SET teacher_id = {teacher_id}
                            WHERE course_id = {course_id}
                        """
            self.c.execute(query_assign_teacher)
            self.mydb.commit()
    ...
    def UpdateCourseInfo(self, course_id, course_name,credits, teacher_id):
        try:
            self.open()
            query = f"""
            ...
            raise InvalidCourseDataException("Invalid data provided for updating the course.")
        finally:
            self.close()
    ...
    def UpdateTeacherInfo(self, teacher_id, first_name, last_name,email):
        try:
            if not email or not first_name or not last_name:
                raise InvalidTeacherDataException("Invalid teacher data provided.")
